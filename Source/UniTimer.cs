...
public class UniTimer
{
    ...
    public static UniTimer Register(float duration, Action onComplete, Action<float> onUpdate = null,
        bool isLooped = false, bool smoothLoop = true, bool useRealTime = false, MonoBehaviour autoDestroyOwner = null)
    {
        ...
        if (!_manager)
        {
            var managerInScene = UnityEngine.Object.FindObjectOfType<UniTimerManager>();
            if (managerInScene)
            {
            ...
            else
            {
                ...
                _manager = managerObject.AddComponent<UniTimerManager>();
            }
        }

        var timer = new UniTimer(duration, onComplete, onUpdate, isLooped, smoothLoop, useRealTime, autoDestroyOwner);
    ...
    public static void Cancel(UniTimer timer)
    {
        if (timer != null)
        {
            timer.Cancel();
    ...
    public static void Pause(UniTimer timer)
    {
        if (timer != null)
        {
            timer.Pause();
    ...
    public static void Resume(UniTimer timer)
    {
        if (timer != null)
        {
            timer.Resume();
    ...
    private static UniTimerManager _manager;
    ...
    private readonly bool _smoothLoop;

    #endregion

    #region Private Constructor (use static Register method to create new timer)

    private UniTimer(float duration, Action onComplete, Action<float> onUpdate,
    ...
    internal void Update()
    {
        ...
        if (isPaused)
        {
            _startTime += worldTime - _lastUpdateTime;
        ...
        if (worldTime >= fireTime)
        {
            ...
            if (isLooped)
            {
                _startTime = _smoothLoop ? fireTime : worldTime;
            }
            else
            {
                isCompleted = true;
